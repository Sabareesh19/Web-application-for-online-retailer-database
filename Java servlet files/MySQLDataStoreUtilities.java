...
public class MySQLDataStoreUtilities {
	...
	public static boolean mapEmployeeToStore(String empid){
		boolean bool = false;
		String [] stores = {"S111","S222","S333"};
		try{
			if(getConnection() == 1){
				String insertUser = "INSERT INTO Worksfor(emp_id,store_id) "+ "VALUES (?,?)";
				PreparedStatement pst = conn.prepareStatement(insertUser);
				int rnd = new Random().nextInt(stores.length);
				pst.setString(1,empid);
				pst.setString(2,stores[rnd]);
				pst.executeUpdate();	
	...
	public static boolean insertProduct(Product product,String store_id){
		...
		public static HashMap<Integer, Product> getProductsForStore(String store_id){
			try{
				if(getConnection() == 1){
					HashMap<Integer, Product> users=new HashMap<Integer, Product>();
					String selectUserQuery ="select distinct * from Product,inventory where product.ProductID = inventory.prod_id and store_id ='"+store_id+"'";
					PreparedStatement pst = conn.prepareStatement(selectUserQuery);
					ResultSet rs = pst.executeQuery();
					int count = 0;
					while(rs.next()){
					...
					System.out.println("Displaying store products, displaying total :"+users.size());
					pst.close();
					conn.close();
					return users;
